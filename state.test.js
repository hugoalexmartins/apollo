	...
	try {
		...
		for (let index = 0; index < 30; index += 1) {
			recordCycleEvaluation({
				...
				candidates: [
					{ pool: `pool-${index}`, deterministic_score: 50 + index },
				],
			});
		}
		...
		const persisted = JSON.parse(
			fs.readFileSync(path.join(tempDir, "state.json"), "utf8"),
		);
...
test("state tracks explicit out-of-range direction", () => {
	...
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-direction-test-"),
	);
...
test("state falls back to backup snapshot when primary is missing", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-backup-test-"),
	);
...
test("syncOpenPositions skips auto-close when unresolved workflow references position or pool", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-sync-guard-test-"),
	);
	...
	try {
		...
		persisted.positions["pos-guard-1"].deployed_at = new Date(
			Date.now() - 30 * 60 * 1000,
		).toISOString();
...
test("syncOpenPositions auto-closes missing position when no unresolved workflow exists", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-sync-autoclose-test-"),
	);
	const journalPath = path.join(tempDir, "data", "workflow-actions.jsonl");
	...
	try {
		...
		const statePath = path.join(tempDir, "state.json");
		const persisted = JSON.parse(fs.readFileSync(statePath, "utf8"));
		persisted.positions["pos-close-1"].deployed_at = new Date(
			Date.now() - 30 * 60 * 1000,
		).toISOString();
		fs.writeFileSync(statePath, JSON.stringify(persisted, null, 2));
...
test("updatePnlAndCheckExits ignores stale pnl updates", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-stale-pnl-test-"),
	);
...
test("recordClose can suppress failed deploy positions from live adoption", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-suppress-live-test-"),
	);
	...
	try {
		...
		fs.mkdirSync(path.join(tempDir, "logs"), { recursive: true });
		trackPosition({
			position: "pos-suppress-1",
			pool: "pool-suppress-1",
			pool_name: "Pool Suppress One",
			strategy: "spot",
			amount_sol: 0.5,
			active_bin: 10,
			initial_value_usd: 100,
		...
		recordClose(
			"pos-suppress-1",
			"wide-range deploy failed before liquidity add",
			{
		...
		assert.equal(persisted.positions["pos-suppress-1"].closed, true);
		assert.equal(
			persisted.positions["pos-suppress-1"].suppress_live_adoption,
			true,
		);
	} finally {
		process.chdir(originalCwd);
...
test("syncOpenPositions skips auto-close when manual-review workflow references position or pool", () => {
	const originalCwd = process.cwd();
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-state-sync-manual-review-test-"),
	);
	const journalPath = path.join(tempDir, "data", "workflow-actions.jsonl");
	...
	try {
		...
		const statePath = path.join(tempDir, "state.json");
		const persisted = JSON.parse(fs.readFileSync(statePath, "utf8"));
		persisted.positions["pos-manual-1"].deployed_at = new Date(
			Date.now() - 30 * 60 * 1000,
		).toISOString();
		fs.writeFileSync(statePath, JSON.stringify(persisted, null, 2));

		appendActionLifecycle({
			workflow_id: "cycle-manual:close_position:1",
			lifecycle: "manual_review",
			tool: "close_position",
			cycle_id: "cycle-manual",
			action_id: "cycle-manual:close_position:1",
			position_address: "pos-manual-1",
			pool_address: "pool-manual-1",
		...
		assert.equal(after.positions["pos-manual-1"].closed_at, null);
	} finally {
		setActionJournalPathForTests(null);
		process.chdir(originalCwd);
		fs.rmSync(tempDir, { recursive: true, force: true });
