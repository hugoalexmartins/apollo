...
test("classifyManagementModelGate keeps model usage instruction-only", () => {
	assert.deepEqual(
		classifyManagementModelGate({ instruction: "close if moon soon" }),
		{
			route: "model",
...
test("deploy governance helpers centralize exposure and live deploy checks", () => {
	...
	const tokenOnlySpotAdmission = evaluateDeployAdmission({
		config: {
			screening: { minBinStep: 80, maxBinStep: 125 },
			management: { deployAmountSol: 0.5, gasReserve: 0.1 },
			risk: { maxPositions: 3, maxDeployAmount: 50 },
	...
	assert.equal(missingAmountAdmission.pass, false);
	assert.equal(
		missingAmountAdmission.code,
		DEPLOY_GOVERNANCE_CODES.MISSING_DEPLOY_AMOUNT,
	);
});
