...
test("screening thesis finalizes finalist deploy decisions into executable args", () => {
	const parsed = parseDecisionThesisContent(
		JSON.stringify({
			action: "deploy",
			...
			evidence: [
				{
					source: "ranking",
					summary: "top deterministic score",
...
test("screening thesis forwards resolved preset strategy bins from the selected finalist", () => {
	const parsed = parseDecisionThesisContent(
		JSON.stringify({
			action: "deploy",
			selected_pool: "pool-1",
			summary: "Deploy into the preset-qualified finalist.",
			confidence: { score: 0.84, label: "high" },
			evidence: [
				{
					source: "ranking",
					summary: "preset-qualified finalist",
					supports_action: true,
					freshness: "fresh",
				},
			],
			...
			contradictions: [],
			invalidation_conditions: ["hard block appears"],
		}),
		{
			cycle_id: "screening-preset-1",
			decision_mode: "model",
		},
	);
		...
		finalists: [
			{
				...
				deterministic_score: 91,
				hard_blocked: false,
				deploy_strategy: "spot",
				deploy_bins_below: 24,
				deploy_bins_above: 24,
				deploy_preset_id: "quality_spot",
				deploy_preset_name: "Quality Spot",
				deploy_spot_subtype: "spot_single_sided_sol",
				deploy_deposit_sidedness: "single_sided",
				deploy_semantics_label:
					"spot_single_sided_sol / single-sided sol / two-sided-range",
			},
		],
	});

	assert.equal(thesis.args.strategy, "spot");
	assert.equal(thesis.args.bins_below, 24);
	assert.equal(thesis.args.bins_above, 24);
	assert.equal(thesis.deploy_preset_id, "quality_spot");
	assert.equal(thesis.deploy_spot_subtype, "spot_single_sided_sol");
	assert.equal(thesis.deploy_deposit_sidedness, "single_sided");
	assert.match(thesis.deploy_semantics_label || "", /single-sided sol/i);
});

test("screening thesis becomes inconsistent when selected pool is blocked or missing", () => {
	const blockedParsed = parseDecisionThesisContent(
		JSON.stringify({
			action: "deploy",
			selected_pool: "pool-1",
	...
	assert.ok(
		blockedThesis.contradictions.includes("selected_pool_hard_blocked"),
	);

	const missingParsed = parseDecisionThesisContent(
		JSON.stringify({
			action: "deploy",
	...
	assert.ok(
		missingThesis.contradictions.includes("selected_pool_not_in_finalists"),
	);
});
