...
export function markOutOfRange(position_address, direction = null) {
	...
	log(
		"state",
		`Position ${position_address} marked out of range${pos.out_of_range_direction ? ` (${pos.out_of_range_direction})` : ""}`,
	);
}
...
export function recordClose(
	position_address,
	reason,
	{ suppressLiveAdoption = false } = {},
) {
	const state = load();
	const pos = state.positions[position_address];
	if (!pos) return;
	pos.closed = true;
	pos.suppress_live_adoption = Boolean(suppressLiveAdoption);
...
export function syncOpenPositions(observed_open_positions) {
	...
	const unresolvedWriteWorkflows = foldActionJournal(journal.entries).filter(
		(workflow) => {
			if (!workflow || !workflow.tool) return false;
			if (workflow.lifecycle === "completed") return false;
			return (
				workflow.lifecycle === "intent" ||
				workflow.lifecycle === "close_observed_pending_redeploy" ||
				workflow.lifecycle === "manual_review"
			);
		},
	);
	...
	if (journal.parse_errors.length > 0) {
		log(
			"state_warn",
			`State sync skipped auto-close because action journal has ${journal.parse_errors.length} parse error(s)`,
		);
	}
	...
	for (const posId in state.positions) {
		...
		if (journal.parse_errors.length > 0) {
			log(
				"state_warn",
				`Position ${posId} missing on-chain but action journal is invalid; skipping auto-close`,
			);
			continue;
		}

		const relatedWorkflows = unresolvedWriteWorkflows.filter((workflow) => {
			if (workflow.position_address && workflow.position_address === posId)
				return true;
			if (
				workflow.pool_address &&
				pos.pool &&
				workflow.pool_address === pos.pool
			)
				return true;
			return false;
		});
		...
		if (relatedWorkflows.length > 0) {
			const workflowHints = relatedWorkflows
				...
				.map(
					(workflow) =>
						`${workflow.workflow_id}:${workflow.tool}:${workflow.lifecycle}`,
				)
				.join(", ");
			log(
				"state_warn",
				`Position ${posId} missing on-chain but unresolved workflow(s) exist (${workflowHints}); skipping auto-close`,
			);
			continue;
		}
