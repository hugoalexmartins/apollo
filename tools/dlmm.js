...
export function shouldIgnoreTrackedLivePosition(trackedPosition = null) {
	return Boolean(
		trackedPosition?.closed && trackedPosition?.suppress_live_adoption,
	);
}

export function finalizeCommittedWriteResult(baseResult, warnings = []) {
	...
	return {
		...
		manual_review_reason: warnings[0],
	};
}

function appendCommittedWriteWarning(warnings, logType, prefix, error) {
...
export async function chooseDistributionStrategy({
	pool_data = {},
	expected_volume_profile = "balanced",
}) {
	return chooseDistributionStrategyPlan({ pool_data, expected_volume_profile });
}

export async function calculateDynamicBinTiers({
	six_hour_volatility,
	trend_bias = "neutral",
}) {
...
export async function deployPosition({
	...
	if (!deployAdmission.pass) {
		if (
			deployAdmission.code === DEPLOY_GOVERNANCE_CODES.PORTFOLIO_GUARD_ACTIVE
		) {
		...
		if (
			deployAdmission.code ===
			DEPLOY_GOVERNANCE_CODES.POOL_LOW_YIELD_COOLDOWN_ACTIVE
		) {
			return {
				success: false,
				blocked: true,
	...
	let suppressLiveAdoptionOnFailure = false;
	let trackedPositionPersisted = false;
	const persistenceWarnings = [];

	const persistTrackedPositionSafely = (context) => {
		if (trackedPositionPersisted) return;
		try {
			persistTrackedPosition();
			trackedPositionPersisted = true;
		} catch (error) {
			appendCommittedWriteWarning(
				persistenceWarnings,
				"deploy_warn",
				`Local position tracking failed ${context}`,
				error,
			);
		}
	};
	...
	try {
		...
		if (isWideRange) {
			...
			for (let i = 0; i < createTxArray.length; i++) {
				...
				const txHash = await sendAndConfirmTransaction(
					...
					createTxArray[i],
					signers,
					buildSafeSendOptions(),
				);
				txHashes.push(txHash);
				if (!suppressLiveAdoptionOnFailure) {
					persistTrackedPositionSafely("after empty-position creation");
					suppressLiveAdoptionOnFailure = trackedPositionPersisted;
				}
			...
			for (let i = 0; i < addTxArray.length; i++) {
				...
				txHashes.push(txHash);
				suppressLiveAdoptionOnFailure = false;
				log(
					"deploy",
			...
			txHashes.push(txHash);
			persistTrackedPositionSafely("after deploy confirmation");
		}

		log("deploy", `SUCCESS — ${txHashes.length} tx(s): ${txHashes[0]}`);

		_positionsCacheAt = 0;
		persistTrackedPositionSafely("after all deploy transactions confirmed");

		const actualBinStep = pool.lbPair.binStep;
		const activePrice = parseFloat(activeBin.price);
		const minPrice =
			activePrice * (1 + actualBinStep / 10000) ** (minBinId - activeBin.binId);
		const maxPrice =
			activePrice * (1 + actualBinStep / 10000) ** (maxBinId - activeBin.binId);

		return finalizeCommittedWriteResult({
			success: true,
			position: positionAddress,
			pool: pool_address,
			pool_name,
		...
		if (isWideRange && suppressLiveAdoptionOnFailure) {
			try {
				recordClose(
					positionAddress,
					"wide-range deploy failed before liquidity was added",
					{
						suppressLiveAdoption: true,
					},
				);
			} catch (stateError) {
				log(
					"deploy_warn",
					`Failed to suppress orphaned wide-range position ${positionAddress}: ${stateError.message}`,
				);
			}
		}
...
export async function getPositionPnl({ pool_address, position_address }) {
	...
	try {
		...
		if (!p)
			return {
				error: "Position not found in PnL API",
				stale: true,
				status: "stale",
			};
		const observedAtMs = Number.isFinite(
			Number(
				p.observed_at_ms ?? p.as_of_ms ?? p.updatedAtMs ?? p.updated_at_ms,
			),
		)
			? Number(
					p.observed_at_ms ?? p.as_of_ms ?? p.updatedAtMs ?? p.updated_at_ms,
	...
	_positionsInflight = (async () => {
		try {
			...
			for (const acc of accounts) {
				...
				const lbPairKey = new PublicKey(
					acc.account.data.slice(8, 40),
				).toBase58();
				const tracked = getTrackedPosition(positionAddress);
				if (shouldIgnoreTrackedLivePosition(tracked)) continue;
			...
			const pnlMaps = await Promise.all(
				uniquePools.map((pool) => fetchDlmmPnlForPool(pool, walletAddress)),
			);
			const pnlByPool = {};
			...
			uniquePools.forEach((pool, i) => {
				...
				if (mintMap?.error) {
					log(
						"positions_warn",
						`Pool mint enrichment failed for ${pool.slice(0, 8)}: ${mintMap.error}`,
					);
			...
			const positions = raw.map((r) => {
				...
				const ageFromState = tracked?.deployed_at
					...
					: null;
				const ageMinutes =
					Math.max(ageFromPnlApi ?? 0, ageFromState ?? 0) || null;
				...
				return {
					...
					stale:
						p?.stale === true || p?.status === "stale" || staleWithoutFreshness,
					status: pnlMissing
						? "missing"
						: p?.status || (staleWithoutFreshness ? "stale" : "ok"),
					observed_at_ms: observedAtMs,
			...
			log(
				"positions_error",
				`SDK scan failed: ${error.stack || error.message}`,
			);
			return {
				wallet: walletAddress,
				total_positions: 0,
...
export async function rebalanceOnExit({
	...
	const context = await getPositionExecutionContext(position_address, {
		getMyPositions,
		getPositionPnl,
		buildTrackedFallback: (address) =>
			buildTrackedPositionFallback(address, { getTrackedPosition }),
		resolveBinSnapshot,
		classifyRangeLocation,
	...
	const outOfRange =
		context.in_range === false || context.range_location.in_range === false;
	if (!outOfRange && !force_rebalance) {
		return {
			...
			reason:
				"Position is currently in range. Set force_rebalance=true to override.",
			range_snapshot: context.bin_snapshot,
			range_location: context.range_location,
		};
	}

	const planningData = buildPoolPlanningData(
		context.position,
		context.bin_snapshot,
	);
	const expectedVolumeProfile =
		expected_volume_profile || planningData.expected_volume_profile;
	...
	const binsAbove =
		strategy === "bid_ask"
			? 0
			: clampWholeNumber(
					tierPlan?.range_plan?.bins_above ?? 0,
					0,
					MAX_BINS_PER_SIDE,
				);
	...
	if (
		!poolTokenMints ||
		poolTokenMints.error ||
		!poolTokenMints.token_x_mint ||
		!poolTokenMints.token_y_mint
	) {
		return {
			success: false,
	...
	const recoveredDeltaPlan = calculateBalanceDeltas(
		beforeCloseSnapshot,
		afterCloseSnapshot,
	);
	...
	if (strategy === "bid_ask" && recoveredDeltaPlan.amount_y <= 0) {
		return {
			success: false,
			error:
				"Closed position but bid_ask redeploy requires quote-side amount_y > 0",
			position: position_address,
			close_result: closeResult,
			planning: {
				reinvestment_plan: {
	...
	if (
		...
		!Array.isArray(remainingPositions?.positions)
	) {
		return {
			success: false,
	...
	const redeployAdmission = evaluateDeployAdmission({
		...
		poolCooldown: getPoolDeployCooldown({
			pool_address: deployArgs.pool_address,
		}),
	});
...
export async function autoCompoundFees({
	...
	const compoundingLocation = normalizeCompoundingLocation(
		context.range_location,
	);
	...
	const unclaimedFeeUsd = Math.max(
		0,
		toFiniteNumber(
			context.pnl?.unclaimed_fee_usd ?? context.position.unclaimed_fees_usd,
			0,
		),
	);
	...
	const actionPlan = {
		...
		reinvest_plan: {
			...
			blocked_reason:
				"Current primitives do not support safe in-place compounding. Opening another position in the same pool is blocked.",
	...
	if (dryRun) {
		...
		return {
			...
			action_plan: actionPlan,
			message:
				"DRY RUN — claim + compounding plan generated, no transactions sent",
		};
	}
	...
	if (!execute_reinvest) {
		return {
			...
			action_plan: actionPlan,
			message:
				"Fees claimed. In-place compounding is not safely supported by current primitives; returned a non-executed action plan.",
		};
	}
	...
	return {
		...
		claim_result: claimResult,
		reason:
			"execute_reinvest requested, but duplicate-position deployment is blocked and in-place compounding is not safely supported by current primitives.",
		planning: {
			expected_volume_profile: expectedVolumeProfile,
			trend_bias: planningData.trend_bias,
			distribution_plan: distributionPlan,
...
export async function claimFees({ position_address }) {
	...
	if (trackedPosition?.closed) {
		return {
			success: false,
			error: `Position ${position_address} is already closed`,
		};
	}
	if (process.env.DRY_RUN === "true") {
		return {
	...
	try {
		...
		const positionsBeforeClaim = await getMyPositions({ force: true }).catch(
			() => null,
		);
		const previousOpenPosition =
			!positionsBeforeClaim?.error &&
			Array.isArray(positionsBeforeClaim?.positions)
				? positionsBeforeClaim.positions.find(
						(position) => position.position === position_address,
					)
				: null;
		const previousUnclaimedFeeUsd = Number(
			previousOpenPosition?.unclaimed_fees_usd ??
				previousOpenPosition?.unclaimed_fee_usd ??
				null,
		);
		...
		let settlement = {
			settled: false,
			reason: "settlement_not_observed",
			observed_amount_received: 0,
			observed_mint: null,
			observed_unclaimed_fee_delta: null,
			signal: null,
			attempts: 0,
		};
		try {
			settlement = await waitForPostClaimSettlement({
				walletPubkey: wallet.publicKey,
				observedMints: claimMints,
				positionAddress: position_address,
				previousBalancesByMint,
				previousUnclaimedFeeUsd: Number.isFinite(previousUnclaimedFeeUsd)
			...
			if (!settlement.settled) {
				appendCommittedWriteWarning(
					persistenceWarnings,
					"claim_warn",
					"Post-claim settlement not observed before timeout",
					new Error(settlement.reason),
				);
			}
		} catch (error) {
			appendCommittedWriteWarning(
				persistenceWarnings,
				"claim_warn",
				"Post-claim settlement verification failed",
				error,
			);
		}
		try {
...
export async function closePosition({
	...
	try {
		...
		let settlement = {
			settled: false,
			reason: "settlement_not_observed",
			observed_balance_delta: 0,
		};
		try {
			settlement = await waitForPostCloseSettlement({
				walletPubkey: wallet.publicKey,
				baseMint,
				positionAddress: position_address,
			...
			if (!settlement.settled) {
				appendCommittedWriteWarning(
					persistenceWarnings,
					"close_warn",
					"Post-close settlement not observed before timeout",
					new Error(settlement.reason),
				);
			}
		} catch (error) {
			appendCommittedWriteWarning(
				persistenceWarnings,
				"close_warn",
				"Post-close settlement verification failed",
				error,
			);
		}
		...
		try {
			...
			appendCommittedWriteWarning(
				persistenceWarnings,
				"close_warn",
				"Local close recording failed",
				error,
			);
		}

		let tracked = null;
		let closePerformance = null;
		_positionsCacheAt = 0;
		try {
			tracked = getTrackedPosition(position_address);
			if (tracked) {
				...
				closePerformance = buildClosePerformancePayload({
					tracked,
					cachedPosition: _positionsCache?.positions?.find(
						(position) => position.position === position_address,
					),
					poolAddress,
					pool,
			...
			appendCommittedWriteWarning(
				persistenceWarnings,
				"close_warn",
				"Close performance recording failed",
				error,
			);
		}
