...
function computePortfolioEquityUsd(portfolioSnapshot = {}) {
  ...
  const fromSolUsd = Number.isFinite(solUsd)
    ? solUsd
    : Number(portfolioSnapshot?.sol || 0) * Number(portfolioSnapshot?.sol_price || 0);
...
function updateEquitySnapshots(state, equityUsd, nowIso) {
  if (!Number.isFinite(equityUsd) || equityUsd <= 0) {
    return {
      equity_usd: 0,
      equity_high_watermark_usd: Number(state.metrics?.equity_high_watermark_usd || 0),
      drawdown_pct: 0,
    };
  }

  const snapshots = Array.isArray(state.equity_snapshots) ? state.equity_snapshots : [];
  ...
  const drawdownPct = highWatermark > 0
    ? ((highWatermark - equityUsd) / highWatermark) * 100
    : 0;
...
function buildTrigger(metrics) {
  ...
  if (protections.maxDrawdownPct > 0 && metrics.drawdown_pct >= protections.maxDrawdownPct) {
    return {
  ...
  if (protections.maxOpenUnrealizedLossUsd > 0 && metrics.open_unrealized_loss_usd >= protections.maxOpenUnrealizedLossUsd) {
    return {
  ...
  if (protections.stopLossStreakLimit > 0 && metrics.stop_loss_streak >= protections.stopLossStreakLimit) {
    return {
  ...
  if (protections.maxRecentRealizedLossUsd > 0 && metrics.recent_loss_usd >= protections.maxRecentRealizedLossUsd) {
    return {
...
export function evaluatePortfolioGuard({ nowMs = Date.now(), portfolioSnapshot = null, openPositionPnls = [] } = {}) {
  ...
  const equityMetrics = updateEquitySnapshots(state, computePortfolioEquityUsd(portfolioSnapshot), nowIso);
  ...
  const pauseUntil = new Date(nowMs + protections.pauseMinutes * 60_000).toISOString();
...
export function clearPortfolioGuardPause({ reason = "operator override", nowMs = Date.now() } = {}) {
  const state = loadGuardState();
...
export function formatPortfolioGuardReport(status = getPortfolioGuardStatus()) {
  ...
  if (status.reason_code) lines.push(`  reason_code: ${status.reason_code}`);
    ...
    lines.push(`  equity_high_watermark_usd: ${status.metrics.equity_high_watermark_usd}`);
    lines.push(`  drawdown_pct: ${status.metrics.drawdown_pct}`);
    lines.push(`  open_unrealized_loss_usd: ${status.metrics.open_unrealized_loss_usd}`);
    lines.push(`  open_positions_considered: ${status.metrics.open_positions_considered}`);
  }
  lines.push("");
  return lines.join("\n");
}
