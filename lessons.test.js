...
test("recordPerformance exposes fee and inventory attribution", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-lessons-test-"),
	);
	...
	try {
		...
		fs.writeFileSync(
			userConfigPath,
			JSON.stringify({ minFeeActiveTvlRatio: 0.05, minOrganic: 60 }, null, 2),
		);
		...
		} = await import(`./lessons.js?test=${Date.now()}`);
		...
		assert.equal(
			persisted.performance[0].closed_by_action_id,
			"management-456:close_position:1",
		);
		...
		if (originalLessonsFile)
			process.env.ZENITH_LESSONS_FILE = originalLessonsFile;
		else delete process.env.ZENITH_LESSONS_FILE;
		if (originalRolloutFile)
			process.env.ZENITH_THRESHOLD_ROLLOUT_FILE = originalRolloutFile;
...
test("evolveThresholds writes current screening keys only", async () => {
	...
	try {
		...
		const { evolveThresholds } = await import(
			`./lessons.js?test=${Date.now()}`
		);

		const perfData = [
			{ pnl_pct: 8, fee_tvl_ratio: 0.3, organic_score: 84 },
		...
		assert.equal(
			rollout.active.changed_keys.includes("minFeeActiveTvlRatio"),
			true,
		);
...
test("recordPerformance auto-derives inactive strategy candidates every 5 closes", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-auto-strategy-candidate-test-"),
	);
	const originalCwd = process.cwd();
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	...
	try {
		...
		const { recordPerformance } = await import(
			`./lessons.js?test=${Date.now()}`
		);
		...
		for (let index = 0; index < 5; index += 1) {
			await recordPerformance({
				position: `pos-${index}`,
				pool: `pool-${index}`,
				pool_name: `Alpha-${index}`,
				strategy: "spot",
				bin_range: { min: 10, max: 46, bins_below: 18, bins_above: 18 },
				bin_step: 85,
				volatility: 4,
				fee_tvl_ratio: 0.11,
				organic_score: 84,
		...
		const strategyLibrary = JSON.parse(
			fs.readFileSync(path.join(tempDir, "strategy-library.json"), "utf8"),
		);
		const candidate = strategyLibrary.strategies.auto_spot_candidate;
		assert.ok(candidate);
		assert.equal(strategyLibrary.active, null);
		assert.equal(candidate.source, "auto_derived");
		assert.equal(candidate.status, "candidate");
		assert.equal(candidate.auto_activate, false);
		assert.equal(candidate.lp_strategy, "spot");
		assert.equal(candidate.entry.single_side, "sol");
		assert.equal(candidate.range.bins_below, 18);
		assert.equal(candidate.range.bins_above, 18);
		assert.equal(candidate.evidence.positions_analyzed, 5);
		assert.equal(candidate.evidence.review_status, "manual_review_required");
	} finally {
		process.chdir(originalCwd);
		if (originalUserConfigPath)
			process.env.ZENITH_USER_CONFIG_PATH = originalUserConfigPath;
...
test("evolveThresholds rolls back pending rollout on degraded post-change performance", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-rollback-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
...
test("evolveThresholds accepts rollout when post-change performance stays healthy", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-accept-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
	...
	try {
		...
		const { config } = await import(`./config.js?test=${Date.now()}`);
		const { evolveThresholds, getThresholdRolloutState } = await import(
			`./lessons.js?test=${Date.now()}`
		);
...
test("evolveThresholds does not falsely block after config mutation when later lesson append fails", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-partial-write-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
...
test("recoverThresholdRolloutState blocks invalid screening values in pending rollout state", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-invalid-values-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
	const userConfigPath = path.join(tempDir, "user-config.json");
	...
	try {
		...
		const { config } = await import(`./config.js?test=${Date.now()}`);
		const { recoverThresholdRolloutState } = await import(
			`./lessons.js?test=${Date.now()}`
		);
		const result = recoverThresholdRolloutState(config, {
			trigger: "recovery",
		});
		assert.equal(result.rollout.status, "blocked_invalid_state");
		assert.equal(result.rollout.reason_code, "EVOLVE_ROLLOUT_STATE_INVALID");
	} finally {
		if (originalUserConfigPath)
			process.env.ZENITH_USER_CONFIG_PATH = originalUserConfigPath;
		else delete process.env.ZENITH_USER_CONFIG_PATH;
...
test("recoverThresholdRolloutState fails closed on malformed parseable active rollout state", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-invalid-shape-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
	const userConfigPath = path.join(tempDir, "user-config.json");
	...
	try {
			...
			JSON.stringify(
				{
					active: {
						phase: "mystery_phase",
						rollout_id: "rollout-invalid-shape",
						start_positions_count: 5,
						min_closes_required: 5,
						changed_keys: ["minFeeActiveTvlRatio"],
						previous_values: { minFeeActiveTvlRatio: 0.05 },
						new_values: { minFeeActiveTvlRatio: 0.2 },
		...
		const { config } = await import(`./config.js?test=${Date.now()}`);
		const { getThresholdRolloutState, recoverThresholdRolloutState } =
			await import(`./lessons.js?test=${Date.now()}`);
		const state = getThresholdRolloutState();
		assert.equal(state.invalid_state, true);
		assert.match(state.error || "", /invalid shape/i);
...
test("getLessonsForPrompt keeps recent fill role-isolated", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-lessons-role-test-"),
	);
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	...
	try {
		...
		fs.writeFileSync(
			process.env.ZENITH_LESSONS_FILE,
			JSON.stringify(
				{
					lessons: [
						{
		...
		const { getLessonsForPrompt } = await import(
			`./lessons.js?test=${Date.now()}`
		);
		const screener = getLessonsForPrompt({
			agentType: "SCREENER",
			maxLessons: 10,
		});
...
test("getLessonsForPrompt surfaces invalid lessons state explicitly", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-lessons-invalid-prompt-test-"),
	);
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
...
test("recordPerformance sanitizes close reasons before lesson generation", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-lessons-close-reason-test-"),
	);
	const originalCwd = process.cwd();
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	...
	try {
		...
		fs.mkdirSync(path.join(tempDir, "logs"), { recursive: true });
		process.env.ZENITH_LESSONS_FILE = path.join(tempDir, "lessons.json");
		const { recordPerformance, getLessonsForPrompt } = await import(
			`./lessons.js?test=${Date.now()}`
		);
		await recordPerformance({
			position: "pos-bad-1",
		...
		const promptLessons = getLessonsForPrompt({
			agentType: "GENERAL",
			maxLessons: 5,
		});
...
test("recoverThresholdRolloutState finalizes apply_pending state", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-apply-recover-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
	...
	try {
		...
		const recovery = recoverThresholdRolloutState(config, {
			trigger: "startup",
		});
...
test("recoverThresholdRolloutState finalizes rollback_pending state", async () => {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "zenith-rollout-rollback-recover-test-"),
	);
	const originalUserConfigPath = process.env.ZENITH_USER_CONFIG_PATH;
	const originalLessonsFile = process.env.ZENITH_LESSONS_FILE;
	const originalRolloutFile = process.env.ZENITH_THRESHOLD_ROLLOUT_FILE;
	...
	try {
		...
		fs.writeFileSync(
			userConfigPath,
			JSON.stringify({ minFeeActiveTvlRatio: 0.2, minOrganic: 75 }, null, 2),
		);
		fs.writeFileSync(
			lessonsPath,
			JSON.stringify({ lessons: [], performance: [] }, null, 2),
		...
		const { config } = await import(`./config.js?test=${Date.now()}`);
		const { recoverThresholdRolloutState, getThresholdRolloutState } =
			await import(`./lessons.js?test=${Date.now()}`);
